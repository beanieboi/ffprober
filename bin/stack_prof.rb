...
MODES.each do |mode|
  ...
  File.open("tmp/stackprof-from_file-#{mode.to_s}.dump", "wb"){ |f| f.write Marshal.dump(from_file_profile) }

  valid_profile = StackProf.run(mode: mode) do
    Ffprober::FfprobeVersion.valid?
  end
  File.open("tmp/stackprof-valid-#{mode.to_s}.dump", "wb"){ |f| f.write Marshal.dump(valid_profile) }
